#!/usr/bin/env python3
"""/obsidian doctor — self-test the memory system. `--fix` repairs symlink/dirs + migrates drift.
Read-only without --fix. Always exit 0 (it's a report)."""

import os, re, sys, json, glob, subprocess

HOME = os.path.expanduser("~")
MEM = os.environ.get("CLAUDE_MEMORY_DIR") or os.path.join(HOME, ".claude/memory")
HOOKS = os.path.join(HOME, ".claude/hooks")
VAULT_LINK = os.environ.get("CLAUDE_VAULT_LINK", "")
...
chk("Obsidian vault symlink", link_ok, VAULT_LINK or "(CLAUDE_VAULT_LINK unset — skipped)", warn=True)
# writable dirs
...
print("# /obsidian doctor" + ("  (--fix applied)" if FIX else ""))
...
print(
    ...
    + (
        ...
        else "   (run `/obsidian doctor --fix` to repair)"
    )
)
