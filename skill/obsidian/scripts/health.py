...
KNOWN = {"wikilink", "wikilinks", "MEMORY", "context", "_session-log", "_Home"}
KNOWN |= {x.strip() for x in os.environ.get("CLAUDE_KNOWN_DANGLERS", "").split(",") if x.strip()}
STALE_DAYS = 120
notes = [
    p
...
existing = {os.path.splitext(os.path.basename(p))[0] for p in notes} | {
    "MEMORY",
    "context",
    "_session-log",
    "_Home",
...
print(
    f"\nstale (>{STALE_DAYS}d unconfirmed, status=active): {len(stale)} — reconfirm or /obsidian prune"
)
