...
FAIL = re.compile(
    ...
    r"(?m)^fatal:|Permission denied|npm ERR!|returned non-zero|exit code [1-9]|exit status [1-9]|"
...
def main():
    ...
    resp = resp if isinstance(resp, str) else json.dumps(resp) if resp is not None else ""
    ...
    if n == THRESHOLD or (n > THRESHOLD and n % 2 == 0):   # nudge at 2, then 4, 6…
        ...
        print(f"⚠ stuck-check: `{sig}` has failed {n}× in a row. "
              f"Before retrying, pull prior context from memory: `/obsidian pull {kws}` "
